...
public:
    ...
    static LatLng unproject(const Point<double>& p, double scale, LatLng::WrapMode wrapMode = LatLng::Unwrapped) {
        ...
        if (mbgl::util::projectionType == mbgl::util::ProjectionType::MERCATOR_PROJECTION) {
            return LatLng{
                std::atan(std::exp(util::deg2rad(util::LONGITUDE_MAX - p2.y))) * util::DEGREES_MAX / M_PI - 90.0,
    ...
    static Point<double> project_(const LatLng& latLng, double worldSize) noexcept {
        ...
        if (mbgl::util::projectionType == mbgl::util::ProjectionType::MERCATOR_PROJECTION) {
            ...
            return Point<double>{util::LONGITUDE_MAX + latLng.longitude(),
                                 util::LONGITUDE_MAX - util::rad2deg(std::log(
                                                           std::tan(M_PI / 4 + latitude * M_PI / util::DEGREES_MAX)))} *
